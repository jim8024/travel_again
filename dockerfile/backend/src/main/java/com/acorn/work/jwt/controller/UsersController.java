...
public class UsersController {
    ...
    @PostMapping("/auth2")
    public String auth(@RequestBody MemberEntity memberEntity) throws Exception {
        try {
            authManager.authenticate(
                    new UsernamePasswordAuthenticationToken(memberEntity.getMemberId(),memberEntity.getPassword()));
        } catch (Exception e ){
            e.printStackTrace();
