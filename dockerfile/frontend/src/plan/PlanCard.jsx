...
export default function PlanCard({
  ...
  const handleAddButtonClick = (item) => {
    ...
    const updatedTourList = tourList.filter(
      (dataItem) => dataItem.contentid !== item.contentid
    );
  ...
  const handleNextPage = () => {
    const filteredItems = output.filter(
      (item) => item.areacode === areaData.areacode
    );
  ...
  const filteredItems = tourList.filter(
    (item) => item.areacode === areaData.areacode
  );
